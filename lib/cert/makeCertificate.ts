...
export type CertData = {
  ...
  logoUrl?: string | null // p.ej. /images/logo-oretec.png
}
...
export async function makeCertificate(data: CertData): Promise<Blob> {
  ...
  // Logo si existe (no rompe si falla)
  try {
    const res = await fetch(logoUrl)
    if (res.ok) {
      const arr = await res.arrayBuffer()
      const img = await pdfDoc.embedPng(arr).catch(async () => pdfDoc.embedJpg(arr))
      const iw = 120
  ...
  page.drawText('Instructor', {
    x: 210, y: firmaY - 34, size: 10, font: fontBody, color: rgb(0.35,0.35,0.35),
  })

  // QR de verificación
  try {
    const dataUrl = await QRCode.toDataURL(verificacionUrl, { margin: 1, width: 200 })
    const b64 = dataUrl.split(',')[1]
    const bytes = Uint8Array.from(atob(b64), c => c.charCodeAt(0))
    const img = await pdfDoc.embedPng(bytes)
    page.drawImage(img, { x: width - 260, y: 80, width: 180, height: 180 })
    page.drawText('Verificación', {
      x: width - 220, y: 70, size: 10, font: fontBody, color: rgb(0.35,0.35,0.35)
    })
  } catch {/* ignore */}

  // Footer
  page.drawText('Emitido por OreTec', {
    x: 60, y: 60, size: 10, font: fontBody, color: rgb(0.35,0.35,0.35)
  })

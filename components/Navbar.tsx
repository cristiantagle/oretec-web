...
export default function Navbar() {
  ...
  // 🔄 Suscripción a cambios de autenticación (sin F5)
  useEffect(() => {
    const sub = supabase.auth.onAuthStateChange(async (event, session) => {
      try {
        ...
        if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED' || event === 'USER_UPDATED') {
          ...
          // Forzar re-render de rutas que lean cookies/headers (App Router)
          router.refresh()
        }
      } catch {
        // silencioso
    ...
    return () => {
      // compatibilidad con distintas versiones del cliente
      // @ts-ignore

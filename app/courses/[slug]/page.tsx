...
export default function CourseDetailPage() {
  ...
  const participantsParam = search.get('participants')
  ...
  const empresaHref = `/dashboard/company/participants?course=${encodeURIComponent(slug)}`
  const hasParty = decoded && Array.isArray(decoded.participants) && decoded.participants.length > 0
  const unitPrice = typeof price === 'number' ? price : 0
  const subtotal  = hasParty ? (decoded!.participants!.length * unitPrice) : 0
      ...
      <FadeIn>
        ...
        <div className="mt-6 flex gap-3 flex-wrap">
          ...
          <Link
            ...
            title="Agregar participantes y comprar como empresa"
          >
            Compra empresa (agregar participantes)
          </Link>
        ...
        {/* NUEVO: Resumen empresa si vienen participantes en la URL */}
        {hasParty && (
          <section className="mt-8 rounded-2xl border bg-white p-5 shadow-soft">
            <h2 className="text-lg font-semibold text-slate-900 mb-2">Resumen empresa</h2>
            <div className="text-sm text-slate-600 mb-3">
              Participantes: <span className="font-medium">{decoded!.participants!.length}</span> ·
              {' '}Unitario: <span className="font-medium">{clp(unitPrice)}</span> ·
              {' '}Subtotal: <span className="font-semibold">{clp(subtotal)}</span>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="text-sm text-slate-500">
                El total se recalcula según el precio del curso y la cantidad de participantes.
              </div>
              <Link
                href={`/checkout?course=${encodeURIComponent(slug)}&participants=${encodeURIComponent(participantsParam || '')}`}

...
export async function GET(req: Request) {
  ...
  if (error) {
    try {
      ...
      if (urlEnv && anonEnv) {
        const fixedUrl = urlEnv.startsWith('http') ? urlEnv : `https://${urlEnv}`
        const dbAnon = createClient(fixedUrl, anonEnv, { auth: { persistSession: false, autoRefreshToken: false } })
        const res2 = await dbAnon.from('courses').select('*').order('updated_at', { ascending: false })
        data = res2.data || []

...
function RegisterInner() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Construcción ROBUSTA del redirect
    const siteEnv = process.env.NEXT_PUBLIC_SITE_URL
    const site =
      (typeof siteEnv === 'string' && siteEnv.length > 0)
        ? siteEnv
        : (typeof window !== 'undefined' ? window.location.origin : 'https://oretec-web.com')

    // Normaliza: sin slash final doble
    const cleanSite = site.replace(/\/$/, '')
    const emailRedirectTo = `${cleanSite}/dashboard`
    ...
    try {
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          // 🔒 Esto es lo que arregla el problema del redirect en el correo
          emailRedirectTo,
          data: {
            full_name: fullName,
            account_type: type,
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            ...
            className={`btn-secondary flex items-center justify-center gap-2 py-2 ${type === 'individual' ? 'ring-2 ring-blue-300' : ''}`}
          >
          ...
          <button
            ...
            className={`btn-secondary flex items-center justify-center gap-2 py-2 ${type === 'company' ? 'ring-2 ring-blue-300' : ''}`}
          >
...
/** Separé esto para evitar que Next trate de tree-shakear de formas raras en edge/SSR */

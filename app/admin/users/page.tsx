...
export default function AdminUsersPage() {
  ...
  async function onChangeRole(userId: string, nextRole: AccountType) {
    setSavingId(userId)
    setError(null)
    try {
      const { data: sess, error: sessErr } = await supabase.auth.getSession()
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-900 ring-1 ring-blue-200">
                      {accountTypeToEs(u.account_type)}
                    </span>
                    <select
                      className="rounded border px-2 py-1 text-sm"
                      value={u.account_type ?? 'student'}
                      onChange={(e) => onChangeRole(u.id, e.target.value as AccountType)}
